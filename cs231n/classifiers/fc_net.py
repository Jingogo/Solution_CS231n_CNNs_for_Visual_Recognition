    ...
    def loss(self, X, y=None):
        ...
        if self.normalization == None:
            for i in reversed(range(self.num_layers)):
                if i<(self.num_layers-1) and self.use_dropout:
                    dout, dW, db = affine_relu_backward(dout, 
                                                        cache_all_layers[i],
                                                        use_dropout=True)
                else:
                    dout, dW, db = affine_backward(dout, 
                                                        cache_all_layers[i])          
                W = self.params['W' + str(i+1)]
        ...
        if self.normalization == "batchnorm" or self.normalization == "layernorm":
            for i in reversed(range(self.num_layers)):
                if i==(self.num_layers-1):
                    dout, dW, db = affine_backward(dout, cache_all_layers[i])
                else:
                    dout, dW, db, dgamma, dbeta = affine_relu_backward(dout,
                                                                       cache_all_layers[i],
                                                                       normalization=self.normalization,
